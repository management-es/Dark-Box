...
class ResponderAltaActivity : Activity() {
    ...
    private lateinit var cerrarTicketButton: Button
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        cerrarTicketButton = findViewById(R.id.cerrarTicketButton)
        ...
        if (ticketId.isNotEmpty()) {
            databaseTickets.child(ticketId).addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    ...
                    if (ticket != null) {
                        ...
                        if (usuarioSolicitante != nombreUsuario) {
                            ...
                            cerrarTicketButton.alpha = 0.5f

                            // Crear el AlertDialog
                            AlertDialog.Builder(this@ResponderAltaActivity)
                                .setTitle("Permiso restringido")
                                .setMessage("Este ticket es de importancia ALTA por lo tanto únicamente puede ser cerrado por el usuario solicitante.")
                ...
                override fun onCancelled(databaseError: DatabaseError) {
                    Toast.makeText(this@ResponderAltaActivity, "Error al consultar el ticket: ${databaseError.message}", Toast.LENGTH_SHORT).show()
                }
            })
        }
                            ...
                            FirebaseDatabase.getInstance().getReference("respuestas").child(respuestaId)
                                ...
                                .addOnCompleteListener { task ->
                                    if (task.isSuccessful) {
                                        ...
                                        Toast.makeText(this@ResponderAltaActivity, "Error al guardar respuesta: ${task.exception?.message}", Toast.LENGTH_SHORT).show()
                                    }
                                }
                        }

                        override fun onCancelled(databaseError: DatabaseError) {
                            Toast.makeText(this@ResponderAltaActivity, "Error al verificar respuesta: ${databaseError.message}", Toast.LENGTH_SHORT).show()
                        }
                    })
            } else {
        ...
        // Funcionalidad del botón Historial, se mantiene el uso de HistorialMediaAltaActivity
